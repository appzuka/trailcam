...
This directory contains a PyTorch-based training and inference pipeline that mirrors the Swift toolchain, using a Faster R-CNN object detector trained on COCO-format data (such as Label Studio COCO exports).
...
- Use `--no-pretrained` to start from random weights.
...
- `pytorch/src/train.py`: training loop, checkpoint saving, optional validation split.
- `pytorch/src/infer.py`: loads a checkpoint, runs detection on images, and optionally writes annotated outputs.
- `pytorch/src/ls_backend.py`: Label Studio ML backend using Faster R-CNN on CPU (full resolution) with `/predict`, `/train`, and `/webhook`.
...
## Label Studio backend (CPU / Faster R-CNN / full resolution)
This backend runs inference and training on **CPU** using **Faster R-CNN** without resizing inputs. It listens on `http://localhost:9091` by default.
